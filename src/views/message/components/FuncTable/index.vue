...
<script type="text/ecmascript-6">
   ...
   */
  import CTransfer from '@/views/message/components/Transfer/Index'
  import configUtils from './configUtils'
  import common from '@/views/message/utils/common'
  import dom from '@/views/message/utils/dom'
  import loginUtils from '@/views/message/utils/loginUtils'
    ...
    methods: {
      ...
      fetchWithPage (paramsStr) {
        ...
        this.$http.get(this.url + paramsStr + (paramsStr ? `&` : `?`) + `page=${newPage}&size=${this.pageSize}`).then((data) => {
          ...
          let cdata = data;
          console.log(fetchDataFormat.page.content);
